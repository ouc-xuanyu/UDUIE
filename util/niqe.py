...
def _get_patches_generic(img, patch_size, is_train, stride):
    ...
    img2 = resize(img, (img.shape[0]*0.5,img.shape[1]*0.5))
...
if __name__ == "__main__":
    ...
    #img=Image.open(r'D:\下载\niqe-master\rgb_gradient.png').convert('LA')
    ref = np.array(img)[:,:,0] # ref
   # dis = np.array(Image.open('./test_imgs/bikes_distorted.bmp').convert('LA'))[:,:,0] # dis
    print(img.size,ref.shape)
