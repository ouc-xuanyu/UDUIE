...
def __gaussian_noise(image):
    ...
    # 合并噪声后的通道
    noisy_image = cv2.merge((noisy_b, noisy_g, noisy_r)).astype(np.uint8)
    noisy_image=cv2.cvtColor(noisy_image, cv2.COLOR_BGR2RGB)
    return Image.fromarray(noisy_image)

def __contrast(image):
    contrastEnhancer = ImageEnhance.Contrast(image)
    ImageContrast = contrastEnhancer.enhance(random.uniform(0.85,1.5))
    return ImageContrast

def __gaussian_filter(image):
    image = np.array(image).astype(np.uint8)
    img_denoised = gaussian_filter(image, random.uniform(0,1))
    return Image.fromarray(img_denoised)

def get_aug_transform():
    is_contrast=1
    is_gaussian_filter=random.randint(0,1)
    is_gaussian_noise=random.randint(0,1)
    transform_list = []
    if is_contrast:
        transform_list.append(transforms.Lambda(lambda img: __contrast(img)))
    if is_gaussian_filter:
        transform_list.append(transforms.Lambda(lambda img: __gaussian_filter(img)))
    if is_gaussian_noise:
        transform_list.append(transforms.Lambda(lambda img: __gaussian_noise(img)))
    return transforms.Compose(transform_list)