...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal',
    ...
    if _type == 'context':
        ...
        net = waterencoder(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout,
                                no_antialias=no_antialias, no_antialias_up=no_antialias_up, n_blocks=6, opt=opt)
...
class waterencoder(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6,
        ...
        """
        assert (n_blocks >= 0)
        super(waterencoder, self).__init__()
        self.opt = opt
        if type(norm_layer) == functools.partial:
            use_bias = norm_layer.func == nn.InstanceNorm2d
        else:
        ...
        self.model = nn.Sequential(*model)
        # init_net(net, init_type, init_gain, gpu_ids, initialize_weights=('stylegan2' not in netG))

    def forward(self, input, layers=[], encode_only=False):
        if -1 in layers:
            layers.append(len(self.model))
        if 1 or len(layers) > 0:
            feat = input
            feats = []
            for layer_id, layer in enumerate(self.model):
                # print(layer_id, layer)
            ...
            return feat, feat_tmp #, feats  # return both output and intermediate features
        else:
            """Standard forward"""
            fake = self.model(input)
            return fake

# net = ResnetGenerator(input_nc=3,output_nc = 3)
# print(net)
# x=torch.randn([4, 3, 256, 256])
# net(x,layers=[0, 4, 8, 12, 16])