...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal',
             init_gain=0.02, no_antialias=False, no_antialias_up=False, gpu_ids=[], _type='context',opt=None):
    """Create a generator
    ...
    if _type == 'context':
        if netG == 'resnet_9blocks':
            ...
            net = G_Resnet(input_nc, output_nc, opt.nz, num_downs=2, n_res=n_blocks - 4, ngf=ngf, norm='inst',
                        nl_layer='relu')
        elif netG == 'water_6blocks':
            net = context_Generator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout,
                                no_antialias=no_antialias, no_antialias_up=no_antialias_up, n_blocks=6, opt=opt)
        elif netG == 'water_9blocks':
            net = context_Generator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout,
                                no_antialias=no_antialias, no_antialias_up=no_antialias_up, n_blocks=9, opt=opt)       
        else:
            raise NotImplementedError('Generator model name [%s] is not recognized' % netG)
    elif _type == 'style':
        net = water_Encoder(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout,
                                no_antialias=no_antialias, no_antialias_up=no_antialias_up, n_blocks=6, opt=opt)
...
def define_H(input_nc, netF, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, no_antialias=False,
             gpu_ids=[], opt=None):
    if netF == 'global_pool':
        net = PoolingF()
    elif netF == 'reshape':
        ...
        net = DomainSampleH(use_mlp=False, init_type=init_type, init_gain=init_gain, gpu_ids=gpu_ids, nc=opt.netF_nc)
    elif netF == 'mlp_sample':
        net = DomainSampleH(use_mlp=True, init_type=init_type, init_gain=init_gain, gpu_ids=gpu_ids, nc=opt.netF_nc)
    elif netF == 'strided_conv':
        net = StridedConvF(init_type=init_type, init_gain=init_gain, gpu_ids=gpu_ids)
    else:
        raise NotImplementedError('projection model name [%s] is not recognized' % netF)
            ...
            if num_patches > 0:
                if feat_id < 3:
                    ...
                    if attn_mats is not None:
                        ...
                        """
                        经过 F.unfold() 函数处理后，得到一个新的三维张量，形状为 (B, ks*ks*C, L)，具体含义如下：
                        B：与输入特征图保持一致，仍表示批次大小。
                        ks*ks*C：表示展开后每个窗口的元素总数。其中，ks 是窗口大小（kernel_size），
                        C 是输入特征图的通道数。展开后，每个窗口内的所有像素值（包括不同通道）被连结为一个一维向量，
                        因此每个窗口的向量长度为 ks * ks * C。
                        L：表示特征图上总共生成了多少个这样的窗口。具体的窗口数量取决于输入特征图的尺寸 (H, W)、当stride=1，为HW个
                        综上所述，F.unfold(feat, kernel_size=k_s, stride=1, padding=3) 
                        这个函数的主要作用是将输入的多维特征图按照给定的窗口大小、步幅和填充进行分解，
                        将每个窗口内的所有像素值（跨通道）连结成一维向量，并将这些向量按顺序组织成一个新的三维张量。
                        这一过程有助于后续对特征图的局部区域特征进行高效提取和处理，常见于诸如注意力机制、空间金字塔池化等深度学习应用中。
                        """
                        L = feat_local.shape[2]  # HW
                        ...
                        prob = torch.where(torch.isinf(prob), torch.full_like(prob, 0), prob)  # (B,L,ks*ks)
                        entropy = torch.sum(torch.mul(attn_local, prob), dim=2)  # (B,L)

                        _, index = torch.sort(entropy)  # A tuple of (sorted_tensor, sorted_indices) is returned,
                    ...
                    x_sample = feat_reshape.flatten(0, 1)  # (B*N,C)
                    patch_id = []
            else:
...
class DomainSampleH(nn.Module):
    def __init__(self, use_mlp=False, init_type='normal', init_gain=0.02, nc=256, gpu_ids=[]):
        # potential issues: currently, we use the same patch_ids for multiple images in the batch
        super(DomainSampleH, self).__init__()
        self.l2norm = Normalize(2)
        self.use_mlp = use_mlp
        self.nc = nc  # hard-coded
        self.mlp_init = False
    ...
    def forward(self, feat):
        ...
        #print(B,C,H,W)
        feat_reshape = feat.permute(0, 2, 3, 1).flatten(1, 2)  # B*HW*C
        x_sample = feat_reshape.flatten(0, 1)#[BHW,C]
        x_sample = self.mlp(x_sample)#[BHW,_C]
        x_sample = self.l2norm(x_sample)
        #--> B H W _C
        #x_sample = x_sample.permute(0, 2, 1).reshape([B, x_sample.shape[-1], H, W])
        return x_sample
...
class context_Generator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6,
        ...
        self.opt = opt
        encoder_blocks = int(math.ceil(n_blocks/2))
        decoder_blocks = n_blocks - encoder_blocks
        self.context_encoder = ResnetEncoder(input_nc, output_nc, ngf=ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=encoder_blocks,
                 padding_type=padding_type, no_antialias=no_antialias)
        self.context_decoder = ResnetDecoder(input_nc, output_nc, ngf=ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=decoder_blocks,
                 padding_type=padding_type, no_antialias=no_antialias_up)

    def forward(self, input, layers=[], encode_only=False):
        feat = self.context_encoder(input, layers, encode_only)
        if encode_only:
            return feat
        output = self.context_decoder(feat)
        return output, feat
    
class water_Encoder(nn.Module):
    """Resnet-based generator that consists of Resnet blocks between a few downsampling/upsampling operations.
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6,
        ...
        """
        assert (n_blocks >= 0)
        super(water_Encoder, self).__init__()
        self.opt = opt
        self.model = ResnetEncoder(input_nc, output_nc, ngf=ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=int(math.ceil(n_blocks/2)),
                 padding_type=padding_type, no_antialias=no_antialias)
